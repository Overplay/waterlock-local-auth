...
function issuePasswordReset( req, res, sails, params ) {
  sails.models.auth.findOne( req.session.resetToken.owner ).populate( 'resetToken' ).exec( function ( err, auth ) {
    ...
    //MAK, Waterlock is an unholy piece of shit that can crash the server ereywhere...including here
    if (!auth.resetToken){
      return res.badRequest({ error: 'not reset token on that auth obj, you\'re fucked.'});
    }
