...
function handlePost( req, res, sails, params ) {
    ...
    if ( !_.isUndefined( params.email ) ) {
        ...
        req.session.resetToken && !_.isUndefined( params.password ) ) {
...
function issuePasswordReset( req, res, sails, params ) {
    ...
    try {
        owner = req.session.resetToken.owner;
    } catch (err){
        sails.log.silly('No owner found in issuePasswordReset');
        return res.forbidden({ error: 'Attempt to reset password without valid session'});
    }

    sails.models.auth.findOne( owner ).populate( 'resetToken' ).exec( function ( err, auth ) {

        if ( err ) {
            return res.serverError( err );
        }

        // MAK, Waterlock is an unholy piece of shit that can crash the server everywhere...including here
        // And the original code in here was just fucking wrong.

        sails.models.auth.update( { id: auth.id }, {
            resetToken: null,
            ...
            // destroy the token
            // save token id before we destroy (MAK: *might) want to check for it's existence, shitheads.)
            var tokenId = req.session && req.session.resetToken && req.session.resetToken.id;
    ...
    if ( params.token ) {
        try {
            ...
            sails.models.auth.findOne( _token.iss ).populate( 'resetToken' ).exec( function ( err, auth ) {
                ...
                var token = auth.resetToken.token;

                auth.resetToken = ''; //one time use
                auth.save();

                if ( authConfig.passwordReset.mail.forwardUrl ) {
                    res.redirect( authConfig.passwordReset.mail.forwardUrl + '?token=' + token );
                } else {
                    res.json( 200 );
                }
