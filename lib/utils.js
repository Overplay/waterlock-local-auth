...
exports.getHtmlEmail = function ( token ) {
    ...
    //MAK: this was a fucktarded chain of relative paths.
    //In our version we expect the jade/pug file to be in /views
    const viewsPath = sails.config.paths.views;
    if ( !viewsPath ) {
        sails.log.error( 'Views path is not properly configured!' );
        sails.log.error( e.message );
        return null;
    }

    var templatePath = path.join( viewsPath, authConfig.passwordReset.template.file );

    try {
        var html = jade.renderFile( templatePath, viewVars );
    } catch ( e ) {
        sails.log.error( 'Failure using Jade to render template. Probably bad path.' );
        sails.log.error(e.message);
        return null;
    }
...
exports.getValidationEmail = function ( token ) {
    ...
    viewVars.url = resetUrl+'&responseType=emailvalidation';
    ...
    var templatePath = path.join( viewsPath, authConfig.validateAccount.template.file );
